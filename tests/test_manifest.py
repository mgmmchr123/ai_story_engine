...
from engine.manifest import build_manifest, load_manifest, pipeline_output_from_context, save_manifest
...
class ManifestTests(unittest.TestCase):
    ...
    def test_pipeline_output_uses_scene_media_duration_total_when_final_duration_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            paths = RunPaths(
                run_dir=root,
            ...
            context.scene_results = {
                1: SceneRenderResult(scene_id=1, status="completed", media_duration_seconds=2.5),
                2: SceneRenderResult(scene_id=2, status="completed", media_duration_seconds=3.0),
            }

            output = pipeline_output_from_context(context)

            self.assertAlmostEqual(output.total_duration_seconds, 5.5, places=3)
